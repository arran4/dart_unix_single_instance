...
final _SocketDart _cSocket = _libc.lookupFunction<_SocketC, _SocketDart>('socket');
...
typedef _AcceptC = Int32 Function(Int32 fd, Pointer<Uint8> addr, Pointer<Uint32> len);
typedef _AcceptDart = int Function(int fd, Pointer<Uint8> addr, Pointer<Uint32> len);
final _AcceptDart _cAccept = _libc.lookupFunction<_AcceptC, _AcceptDart>('accept');
...
typedef _SetsockoptC = Int32 Function(Int32 fd, Int32 level, Int32 optname, Pointer<Uint8> optval, Uint32 optlen);
typedef _SetsockoptDart = int Function(int fd, int level, int optname, Pointer<Uint8> optval, int optlen);
final _SetsockoptDart _cSetsockopt = _libc.lookupFunction<_SetsockoptC, _SetsockoptDart>('setsockopt');
...
final _SendmsgDart _cSendmsg = _libc.lookupFunction<_SendmsgC, _SendmsgDart>('sendmsg');
...
final _CallocDart _cCalloc = _libc.lookupFunction<_CallocC, _CallocDart>('calloc');
...
class AfAlgEncryptor {
  ...
  List<int> process(List<int> data, bool encrypt, {String algType = "skcipher", String algName = "cbc(aes)"}) {
    if (!isSupported) {
    ...
    try {
      ...
      try {
        ...
        Pointer<Uint8> iov = _cCalloc(16, 1); // struct iovec (8 bytes for ptr, 8 for len on 64-bit)
        ...
        Pointer<Int32> cmsg1Level = Pointer.fromAddress(cbuf.address + 8).cast();
        cmsg1Level[0] = _solAlg;
        Pointer<Int32> cmsg1Type = Pointer.fromAddress(cbuf.address + 12).cast();
        cmsg1Type[0] = _algSetOp;
        Pointer<Uint32> cmsg1Data = Pointer.fromAddress(cbuf.address + 16).cast();
        cmsg1Data[0] = encrypt ? _algOpEncrypt : _algOpDecrypt;

        // Second cmsghdr
        int offset = cmsgSpaceOp;
        Pointer<Uint64> cmsg2Len = Pointer.fromAddress(cbuf.address + offset).cast();
        cmsg2Len[0] = 36; // CMSG_LEN(20)
        Pointer<Int32> cmsg2Level = Pointer.fromAddress(cbuf.address + offset + 8).cast();
        cmsg2Level[0] = _solAlg;
        Pointer<Int32> cmsg2Type = Pointer.fromAddress(cbuf.address + offset + 12).cast();
        cmsg2Type[0] = _algSetIv;

        Pointer<Uint32> ivLen = Pointer.fromAddress(cbuf.address + offset + 16).cast();
        ivLen[0] = 16;
        Pointer<Uint8> ivData = Pointer.fromAddress(cbuf.address + offset + 20).cast();
        for (int i = 0; i < 16; i++) {
          ivData[i] = 0; // null IV for simplicity
        }

        // msghdr
        Pointer<Uint8> msg = _cCalloc(56, 1); // struct msghdr is 56 bytes on 64-bit linux
        Pointer<Pointer<Uint8>> msgIov = Pointer.fromAddress(msg.address + 16).cast();
        msgIov[0] = iov;
        Pointer<Uint64> msgIovlen = Pointer.fromAddress(msg.address + 24).cast();
        msgIovlen[0] = 1;
        Pointer<Pointer<Uint8>> msgControl = Pointer.fromAddress(msg.address + 32).cast();
        msgControl[0] = cbuf;
        Pointer<Uint64> msgControllen = Pointer.fromAddress(msg.address + 40).cast();
